...
import { toolHubConfigured } from "@/lib/jarvis/tool-hub";
...
export function getAvaConnections() {
  ...
  const connectedCount = (service: string) => Object.values(accounts[service] || {}).filter((entry) => entry.status === "connected").length;
  ...
  return [
    ...
    { name: "Google Calendar", category: "Schedule", status: connectedCount("googleCalendar") > 0 ? "Credential Found" : "Not Connected", source: "n8n credential catalog" },
    ...
    { name: "Google Sheets", category: "Business data", status: connectedCount("googleSheets") > 0 ? "Credential Found" : "Not Connected", source: "n8n credential catalog" },
