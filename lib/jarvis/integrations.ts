...
export const JARVIS_INTEGRATIONS: JarvisIntegration[] = [
  ...
  { key: "vercel", name: "Vercel", category: "Platform", status: "Connected", permission: "read_only", credentialEnvironmentKeys: ["VERCEL"], capabilities: ["Dashboard hosting", "Server-side API routes"], notes: "Hosts the private Ava dashboard." },
  ...
  { key: "openai", name: "OpenAI", category: "Intelligence", status: "Connected", permission: "execute", credentialEnvironmentKeys: ["OPENAI_API_KEY"], capabilities: ["Assistant reasoning", "Tool-aware responses", "Summarization"], notes: "Uses the server-side Responses API while Ava retains deterministic permission and approval enforcement." },
  ...
  { key: "slack", name: "Slack", category: "Communication", status: "Credential Needed", permission: "requires_approval", credentialEnvironmentKeys: ["SLACK_BOT_TOKEN"], capabilities: ["Read updates", "Draft messages", "Approved sending"], notes: "Requires a dedicated scoped Ava bot credential." },
  { key: "codex", name: "Codex", category: "Development", status: "Connected", permission: "requires_approval", credentialEnvironmentKeys: ["SUPABASE_SERVICE_ROLE_KEY"], capabilities: ["Create task briefs", "Approved local runner queue", "Architecture context"], notes: "Ava queues approved Codex prompt packages for a local Mac runner. Service-role key is used only by the local runner, never in the browser." },
  { key: "business_data", name: "Business Data Sources", category: "Business Intelligence", status: "In Progress", permission: "read_only", credentialEnvironmentKeys: [], capabilities: ["Sales", "Labor", "Catering", "Operations metrics"], notes: "Existing Ava pulses are the first connected business signals." },
