...
import { AVA_CAPABILITIES, avaRealtimeTools, liveAvaCapabilities } from "@/lib/ava/gateway/capabilities";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const context = await compileAvaContext({ supabase, ownerId: auth.identity.ownerId, ownerEmail: auth.identity.email, conversationId: conversation.conversation_id, messages, capabilities: AVA_CAPABILITIES });
    return NextResponse.json({ conversationId: conversation.conversation_id, context, instructions: buildAvaRealtimeInstructions(context), tools: avaRealtimeTools(), capabilities: liveAvaCapabilities(), messages }, { headers: { "Cache-Control": "private, no-store, max-age=0" } });
  } catch (error) {
